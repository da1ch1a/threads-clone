    ...
    <div class="py-2 w-full">
      <div class="flex items-center justify-between">
        ...
        >
          <button
            :disabled="isDeleting"
            class="flex item-center text-white p-1 h-[24px] w-[24px] hover:bg-gray-800 rounded-full cursor-pointer"
          ...
          <div v-if="isMenu" class="absolute border border-gray-600 right-0 z-20 my-1 rounded">
            <button @click="deletePost(post?.id, post?.picture)" class="flex items-center rounded gap-2 text-red-500 justify-between bg-black w-full pl-4 pr-3 py-1 hover:bg-gray-900">
              <div>Delete</div>
              <Icon name="solar:trash==bin-trash-broken" color="#ffffff" size="20" />
            </button>
          </div>
        </div>
      </div>
...
import { type Like, type Post } from '@/models/posts';
